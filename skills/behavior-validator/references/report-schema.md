...
Use this JSON shape when machine-readable behavior validation output is useful.
...
Use `severity` only for failed checks; use `null` otherwise. Redact credentials, tokens, cookies, private user data, and unrelated log content from every evidence field.
