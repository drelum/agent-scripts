...
const charsPerToken = numberArg(argValue("--chars-per-token", "4"), 4);
...
export function newestFilesFirst(files: string[]): string[] {
...
export function isRecentHistoryRecord(
  record: Record<string, unknown>,
  cutoff = cutoffMs,
): boolean {
  if (typeof record.ts !== "number" || !Number.isFinite(record.ts)) return false;
  const timestampMs = record.ts < 1_000_000_000_000 ? record.ts * 1000 : record.ts;
...
function scanUsage(skills: Skill[], logFiles: string[]): Map<string, Usage> {
  ...
  for (const file of logFiles) {
    ...
    try {
      ...
      if (consumedBytes + stat.size > maxLogBytes) continue;
    ...
    const isHistory = path.resolve(file) === path.resolve(home, ".codex/history.jsonl");
    for (const line of text.split(/\r?\n/)) {
      ...
      if (isHistory && !isRecentHistoryRecord(record)) continue;
...
export function tokenCost(text: string, ratio = charsPerToken): number {
  return Math.ceil(Buffer.byteLength(text, "utf8") / numberArg(String(ratio), 4));
}
...
function skillBudget(skills: Skill[], metadataOverheadTokens = 0): Budget {
  ...
  const tokenRatio = charsPerToken;
...
export function canSuggestDeletion(skill: { scope: string }): boolean {
  return skill.scope !== "codex-plugin";
}
...
function duplicateDeleteSuggestions(groups: [string, Skill[]][]): string[] {
  ...
  for (const [name, list] of groups.slice(0, 80)) {
    ...
    const candidates = list
      ...
      .filter(({ skill }) => canSuggestDeletion(skill))
...
function render(
  ...
  const longDescriptions = considered
    ...
    .slice(0, 30);
  const unused = noLogs ? [] : considered
    .filter((skill) => {
  ...
  if (noLogs) lines.push("- unavailable: history scanning disabled by --no-logs");
  else if (unused.length === 0) lines.push("- none");
  lines.push("");
