...
def run_worker(
    ...
    with tempfile.TemporaryDirectory(prefix="visual-inspection-worker-") as temp:
        schema_file = Path(temp) / "schema.json"
        output_file = Path(temp) / "report.json"
        ...
        with (
            ...
            try:
                process = subprocess.Popen(
                    codex_command(repository, schema_file, output_file),
                    cwd=repository,
        ...
        if not output_file.is_file():
            raise VisualInspectionError("Codex worker did not write the structured report")
        return WorkerRun(
...
def _event_message(line: str, step_ids: dict[str, int]) -> str | None:
    ...
    if event_type == "item.completed" and item_type == "agent_message":
        return "worker prepared structured report"
    return None
...
def command_preview(repository: Path) -> list[str]:
    return codex_command(repository, Path("<schema>"), Path("<report>"))
